...
function normalizeOutline(raw: unknown): OutlineSection[] {
  ...
  return sections.map((sec: Record<string, unknown>, idx: number) => ({
    ...
    keyPoints: Array.isArray(sec.keyPoints)
      ? sec.keyPoints.filter((k): k is string => typeof k === "string")
      : [],
...
export async function llmGenerateContent(params: {
  ...
  const outlineBrief = params.outline
    .map((s) => {
      const chapterPts =
        s.subSections.length === 0 ? (s.keyPoints ?? []).slice(0, 3).join("；") : "";
      const chapterLine = chapterPts ? `  [章级要点] ${chapterPts}` : "";
      const subs = s.subSections
        .map((sub) => `  - ${sub.title}：${sub.keyPoints.slice(0, 3).join("；")}`)
        .join("\n");
      return `${s.title}${chapterLine ? `\n${chapterLine}` : ""}${subs ? `\n${subs}` : ""}`;
    })
    .join("\n\n");
...
要求：
1. 发文机关使用「北京经济技术开发区」
2. 采用「北京经济技术开发区关于……的${policyType === "其他" || policyType === "自定义类型" ? "政策" : policyType}」格式（体例为「其他/自定义类型」且未给出具体名称时用「的政策」）
