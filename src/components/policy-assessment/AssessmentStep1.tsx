...
export function AssessmentStep1({ selected, onSelect, onStartAssessment }: Props) {
  ...
  const filtered = LIBRARY_POLICIES.filter(
    (policy) => policy.title.includes(librarySearch) || policy.dept.includes(librarySearch),
  );
      ...
      <section className={`space-y-3 transition-opacity ${uploadSelected ? "opacity-45" : ""}`}>
          <div className="relative">
            <button
              ...
              className="absolute left-2 top-1/2 flex -translate-y-1/2 items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-primary transition-colors hover:bg-primary/5"
            >
            ...
            <input
              ...
              className="h-10 w-full rounded-lg border border-border bg-background pl-[7.5rem] pr-9 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 disabled:cursor-not-allowed disabled:bg-muted/30"
            />
