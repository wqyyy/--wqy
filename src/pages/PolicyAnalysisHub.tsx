...
import { NEW_COMPARE_ANALYSIS_PATH, POLICY_COMPARE_ANALYSIS_TASKS } from "@/lib/policyCompareAnalysisTasks";
...
export default function PolicyAnalysisHub() {
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (!normalized) return POLICY_COMPARE_ANALYSIS_TASKS;
    return POLICY_COMPARE_ANALYSIS_TASKS.filter((task) => task.title.toLowerCase().includes(normalized));
  }, [keyword]);
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 2xl:grid-cols-3">
          {filteredTasks.map((task) => (
            <Card
              ...
              role="button"
              tabIndex={0}
              className="cursor-pointer rounded-[22px] border border-[#e4e6eb] bg-white p-8 shadow-none transition-all hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-[0_12px_32px_rgba(15,23,42,0.05)]"
              onClick={() =>
                navigate(
